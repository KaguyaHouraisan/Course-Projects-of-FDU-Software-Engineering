...
if __name__ == "__main__":
    ...
    while(True):
        #your strategies
        tempInput = strat_random_write_byte(random.choice(inputList)).replace('\0', '')
        if count >= 10000:
            for index in range(0, int(count / 10000)):
                tempInput = tempInput + chr(random.choice(alphabetTable)).replace('\0', '')
                tempInput = strat_random_write_byte(tempInput).replace('\0', '')
        tempRes = run_target('./ls', tempInput, '')[2]
        if tempRes == 2:
            count = count + 1 
            print('Input is: ' + tempInput + ' , false.')
        if tempRes == -6 and tempInput not in inputList:
            print('Input is: ' + tempInput + ' , true.')
            currTime = today.strftime('%Y-%m-%d-%H-%M-%S')
            writeFile(resPath, ('{}  true : ' + tempInput + '\n').format(currTime).encode("utf-8"))
