...
public class BackendHttpRequest {
	...
	public static String backendPut(String url, Map<String, Object> requestBody, HttpHeaders headers) {
		RestTemplate restTemplate = new RestTemplate();
		HttpEntity<Map<String, Object>> httpEntity = new HttpEntity<>(requestBody, headers);
		ResponseEntity<String> response;

		try {
			response = restTemplate.exchange(url, HttpMethod.PUT, httpEntity, String.class, requestBody);
			return response.getBody();
		} catch (HttpClientErrorException e) {
			System.out.println(e.toString());
		}
